...
def main() -> None:
    ...
    if os.path.exists(dl_path):
        ...
        if arguments["--path"]:
            logger.error(
                f"Invalid download path '{dl_path}' specified by --path argument"
            )
        else:
...
def validate_url(client: SoundCloud, url: str) -> str:
    ...
    """
    if url.startswith(
        ("https://m.soundcloud.com", "http://m.soundcloud.com", "m.soundcloud.com")
    ):
        url = url.replace("m.", "", 1)
    if url.startswith(
        (
...
def download_url(client: SoundCloud, kwargs: SCDLArgs) -> typing.Optional[dict]:
    """Detects if a URL is a track or a playlist, and parses the track(s)
    to the track downloader. In API mode, returns a dict for direct download."""
    url = kwargs["l"]
    ...
    def _download_track_api(track, kwargs, playlist_info=None):
        ...
        buffer = None
        content_type = None
        # Download original if possible
        if (
            (
                track.downloadable
                or getattr(track, "user_id", None)
                == getattr(
                    client.get_me() if kwargs.get("auth_token") else None, "id", None
                )
            )
            ...
            url = client.get_track_original_download(
                track.id, getattr(track, "secret_token", None)
            )
            if url:
                r = requests.get(url, stream=True)
                if r.status_code == 401 or r.status_code == 404:
                    pass  # fallback to HLS
                else:
                    header = r.headers.get("content-disposition")
                    params = utils.parse_header(header)
                    if "filename" in params:
                        orig_filename = urllib.parse.unquote(
                            params["filename"][-1], encoding="utf-8"
                        )
                        ...
                        if kwargs.get("flac") and can_convert(orig_filename):
                            ext = ".flac"
                            filename = filename[:-4] + ext
                        # Use re_encode_to_buffer to get bytes
                        buffer = re_encode_to_buffer(
                            track,
                            r,
                            ext[1:] if ext else "mp3",
                            not (kwargs.get("flac") and can_convert(orig_filename)),
                            kwargs,
                            playlist_info=playlist_info,
                            skip_re_encoding=not (
                                kwargs.get("flac") and can_convert(orig_filename)
                            ),
                        )
        ...
        # Fallback to HLS
        # Find best transcoding
        transcodings = [
            t for t in track.media.transcodings if t.format.protocol == "hls"
        ]
        ...
        ext = None
        preset_name = None
        for pn, pe in valid_presets:
            for t in transcodings:
                if t.preset.startswith(pn):
                    transcoding = t
                    ext = pe
                    preset_name = pn
            if transcoding:
    ...
    if api_mode:
        if item is None:
            ...
            return _download_track_api(item, kwargs)
        elif isinstance(item, AlbumPlaylist) or isinstance(item, User):
            return {
                "error": "The provided URL points to a playlist or user collection. This endpoint supports direct download for single track URLs only.",
                "is_unsupported_for_direct_download": True,
            }
        else:
            return {
                "error": f"Unknown item type: {getattr(item, 'kind', str(type(item)))}"
            }
    ...
    if item is None:
        ...
        if kwargs.get("f"):
            ...
            for i, track in itertools.islice(enumerate(tracks, 1), offset, None):
                ...
                download_track(
                    client, track, kwargs, exit_on_fail=kwargs["strict_playlist"]
                )
            ...
            for i, stream_item in itertools.islice(enumerate(items, 1), offset, None):
                ...
                if isinstance(stream_item, (TrackStreamItem, TrackStreamRepostItem)):
                    ...
                    stream_item, (PlaylistStreamItem, PlaylistStreamRepostItem)
                ):
    ...
    with get_filelock(archive):
        with open(archive) as f:
            try:
                ...
                logger.error(
                    "Error trying to convert track ids. Verify archive file is not empty."
                )
                logger.debug(verr)
                sys.exit(1)
        ...
        if rem:
            for track_id in rem:
                ...
                if track is None:
                    logger.warning(
                        f"Could not find track with id: {track_id}. Skipping removal"
                    )
                    continue
...
def get_transcoding_m3u8(
    ...
    if url is not None:
        ...
        while not r or r.status_code == 429:
            ...
            delay = (
                delay or 1
            ) * 2  # exponential backoff, what could possibly go wrong

        if r.status_code != 200:
            raise SoundCloudException(
                f"Unable to get transcoding m3u8({r.status_code}): {r.text}"
            )
...
def download_hls(
    ...
    re_encode_to_out(
        ...
        (
            preset_name if not preset_name.startswith("aac") else "ipod"
        ),  # We are encoding aac files to m4a, so an ipod codec is used
...
def in_download_archive(track: Union[BasicTrack, Track], kwargs: SCDLArgs) -> bool:
    ...
    try:
        with get_filelock(archive_filename), open(
            archive_filename, "a+", encoding="utf-8"
        ) as file:
            file.seek(0)
...
def _try_get_artwork(url: str, size: str = "original") -> Optional[requests.Response]:
    ...
    try:
        artwork_response = requests.get(
            new_artwork_url, allow_redirects=False, timeout=5
        )
...
def _add_metadata_to_stream(
    ...
    album_available: bool = (playlist_info is not None) and not kwargs.get(
        "no_album_tag"
    )
    ...
    try:
        ...
        kwargs_no_sensitive = {
            k: v for k, v in kwargs.items() if k not in ("auth_token",)
        }
        logger.error(
